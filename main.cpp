...
void clear()
{
    ...
    message_caution_texture=NULL;
    message_header_texture=NULL;

    add_doctor_Texture=NULL;

    Ambulance_texture=NULL;
...
int main(int argc,char* args[])
{
    ...
    while(quit)
    {
        while (SDL_PollEvent(&event)!=0)
        {
            ...
            else if (event.type==SDL_MOUSEBUTTONDOWN||event.type==SDL_TEXTINPUT||event.type==SDL_KEYDOWN)
            {
                ...
                else if (menu==0)
                {
                    ...
                    else if (event.motion.x>=1&&event.motion.x<=102 && event.motion.y>=924 && event.motion.y<=975)
                    {
                        quit=false;
                    }
                }
                else if (menu==1)
                {
                    Symptoms();
                    clear();
                    if (event.motion.x>=1254&&event.motion.x<=1411&&event.motion.y>=914&&event.motion.y<=978)
                        {
                            menu=menu-1;
                            first_window();
                        }
                        else if (event.motion.x>=33&&event.motion.x<=175&&event.motion.y>=914&&event.motion.y<=978)
                        {
                            menu=menu+1;
                            Symptoms_help();
                            clear();
                        ...
                        else if (event.motion.x>=648&&event.motion.x<=791&&event.motion.y>=914&&event.motion.y<=978) 
                        {
                            menu=3;
                ...
                else if(menu==2)
                {
                    Symptoms_help();
                    clear();
                    if(event.motion.x>=1326&&event.motion.x<=1434&&event.motion.y>=914&&event.motion.y<=978)
                    {
                        menu=1;
                        Symptoms();
                        clear();
                ...
                else if (menu==3)
                {
                    symptoms_Rend_Background();
                    input_Symptoms();
                    if(event.motion.x>=660&&event.motion.x<=779&&event.motion.y>=914&&event.motion.y<=975) 
                    {
                    ...
                    else if (event.motion.x>=1300&&event.motion.x<=1430&&event.motion.y>=914&&event.motion.y<=975)
                    {
                        clear();
                        menu=1;
                        input_symptoms="";
                ...
                {
                    CMNT_RETURN();
                    if (event.motion.x>=1248&&event.motion.x<=1416&&event.motion.y>=907&&event.motion.y<=970)
                    {
                       clear();
                       menu=0;
                       cmnt="";
                ...
                {
                    message_show_caution();
                    if (event.motion.x>=304 && event.motion.x<=589 && event.motion.y>=658 && event.motion.y<=697 )
                    {
                        menu=19;
                        FILE* f_p=fopen("message.txt","w");
                        fputs(" ",f_p);
                        fclose(f_p);
                        clear();
                        admin_main_menu();
                    ...
                    else if (event.motion.x>=596 && event.motion.x<= 932 && event.motion.y>=658 && event.motion.y<=697 )
                    {
                        menu=19;
                        clear();
                        admin_main_menu();
                    ...
                    else if (event.motion.x>=937 && event.motion.x<=1096 &&event.motion.y>=658 && event.motion.y<=697 )
                    {
                        menu=21;
                        clear();
                        message_function();
                ...
                //add ambulance message caution end
        ...
        SDL_RenderCopy(gRenderer,add_doctor_Texture,NULL,&add_doctor_rect);
        ...
        SDL_RenderCopy(gRenderer,message_header_texture,NULL,&message_header_rect);
        ...
        SDL_RenderCopy(gRenderer,Ambulance_texture,NULL,&Ambulance_rect);
